...
const CAL_NAMESPACE = 'coaching';
const COACHING_CAL_LINK = 'patchapp/coaching';
const COACHING_CAL_URL = 'https://cal.com/patchapp/coaching';
...
function initialiseCoachingEmbed() {
  const calendarElement = document.getElementById('my-cal-inline-coaching');
  ...
  window.Cal.ns[CAL_NAMESPACE]('inline', {
    elementOrSelector: '#my-cal-inline-coaching',
    calLink: COACHING_CAL_LINK,
    config: {
      layout: 'month_view',
...
export default function Coaching() {
  useEffect(() => {
    ...
    document.title = 'Legal speaking coach | Patch';

    const desktopQuery = window.matchMedia('(min-width: 768px)');
    const initialiseForDesktop = (event) => {
      if (event.matches) {
        initialiseCoachingEmbed();
      }
    };
      ...
      <main className="meeting-main">
        <div className="page-shell meeting-shell">
          <header className="meeting-heading">
            <h1>Legal speaking coach</h1>
            <p>
              A unique internship inside the legal sector. Your chance to work with top tier firms
              and their European offices. Let's discuss the position further.
            </p>
          </header>

          <section className="meeting-booking" aria-label="Choose a legal speaking coach call time">
            <div
              className="meeting-calendar"
              id="my-cal-inline-coaching"
              style={{ width: '100%', minHeight: '750px' }}
              aria-label="Choose a legal speaking coach call time"
            />

            <a
              className="meeting-mobile-cta"
              href={COACHING_CAL_URL}
              target="_blank"
              rel="noreferrer"
            >
