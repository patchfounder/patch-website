...
const CAL_NAMESPACE = 'certified';
const CERTIFIED_CAL_LINK = 'patchapp/certified';
const CERTIFIED_CAL_URL = 'https://cal.com/patchapp/certified';
...
function initialiseCertifiedEmbed() {
  const calendarElement = document.getElementById('my-cal-inline-certified');
  ...
  window.Cal.ns[CAL_NAMESPACE]('inline', {
    elementOrSelector: '#my-cal-inline-certified',
    calLink: CERTIFIED_CAL_LINK,
    config: {
      layout: 'month_view',
...
export default function Certified() {
  useEffect(() => {
    ...
    document.title = 'Certification Call | Patch';

    const desktopQuery = window.matchMedia('(min-width: 768px)');
    const initialiseForDesktop = (event) => {
      if (event.matches) {
        initialiseCertifiedEmbed();
      }
    };
      ...
      <main className="meeting-main">
        <div className="page-shell meeting-shell">
          <header className="meeting-heading">
            <h1>Certification Call</h1>
            <p>
              Let's get you certified and start focussing on how we can extract value from your new
              network.
            </p>
          </header>

          <section className="meeting-booking" aria-label="Choose a certification call time">
            <div
              className="meeting-calendar"
              id="my-cal-inline-certified"
              style={{ width: '100%', minHeight: '700px' }}
              aria-label="Choose a certification call time"
            />

            <a
              className="meeting-mobile-cta"
              href={CERTIFIED_CAL_URL}
              target="_blank"
              rel="noreferrer"
            >
